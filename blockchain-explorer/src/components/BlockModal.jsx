...
import { formatTimestamp } from '../services/explorerData';

export default function BlockModal({ block, onClose }) {
  if (!block) return null;

  const blockHeight = block.index ?? block.height ?? 'N/A';
  const prevHash = block.previous_hash || block.previousHash || 'N/A';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center pt-10 pb-10 px-4">
        ...
        <motion.div
          ...
          className="relative max-h-full overflow-y-auto w-full max-w-3xl dashboard-card p-0"
        >
          <div className="sticky top-0 bg-[#0f172a]/95 backdrop-blur-md border-b border-gray-800 p-6 flex justify-between items-center z-10">
            <div>
              <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                <BoxIcon /> Block <span className="text-[#00ff9d]">#{blockHeight}</span>
              </h2>
              <p className="text-gray-400 text-sm mt-1">Mined on {formatTimestamp(block.timestamp)}</p>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <DetailItem icon={<LinkIcon />} label="Previous Hash" value={prevHash} isCode />
                    ...
                    <div key={idx} className="bg-gray-900/50 rounded-lg p-4 border border-gray-800">
                      <div className="flex justify-between mb-3 text-sm">
                        <span className="text-gray-400">
                          ID: <span className="text-gray-300 font-mono">{(tx.id || tx.txID || 'N/A').slice(0, 16)}...</span>
                        </span>
                        <span className="text-[#00ff9d] font-bold">{Number(tx.amount || 0)} NEX</span>
                      </div>
                      <div className="flex items-center gap-4 text-xs font-mono">
                        <div className="flex-1 bg-gray-950 p-2 rounded border border-gray-800 truncate">
                          ...
                          <span className="text-gray-300">{tx.sender || tx.fromAddress || tx.from || 'System'}</span>
                        </div>
                        <div className="text-gray-600">→</div>
                        <div className="flex-1 bg-gray-950 p-2 rounded border border-gray-800 truncate">
                          <span className="text-gray-500 block text-[10px] mb-1">RECEIVER</span>
                          <span className="text-gray-300">{tx.receiver || tx.toAddress || tx.to || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
...
function DetailItem({ icon, label, value, isCode }) {
  return (
    <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-800">
      ...
      <div className={`text-sm text-gray-200 break-all ${isCode ? 'font-mono' : ''}`}>{value || 'N/A'}</div>
    </div>
  );
}
